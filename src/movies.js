...
function howManyMovies(mov) {
  ...
  function filmeDoSteven(film) {
    return film.director === `Steven Spielberg`;
  }
  function filmeDeDrama(film) {
    var drama = false;
    film.genre.forEach(element => {
      if(element===`Drama`){
        drama = true
      }
    });
  ...
  const soma = mov.filter(filmeDoSteven) 
  const soma2 = soma.filter(filmeDeDrama)
  return soma2.length
}
...
function scoresAverage(arr) {
  ...
  const total = score2.reduce((acc,num) => acc += num)
  let result = parseFloat((total/arr.length).toFixed(2))
  return result
}

// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(arr) {
  function howManyMovies2(mov) {
    function filmeDeDrama(film) {
      var drama = false;
      film.genre.forEach(element => {
        if(element===`Drama`)
    ...
    const soma = mov.filter(filmeDeDrama)
    return soma
  }
  const drama = howManyMovies2(arr)
  let media = scoresAverage(drama)
  return media
}

// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(nov) {
  nov.sort((a,b) => (a.year > b.year) ? 1 : ((b.year > a.year) ? -1 : 0))
  return nov
}

// Iteration 6: Alphabetic Order - Order by title and print the first 20 titles
function orderAlphabetically(arr) {
  function orderByName(nov) {
    const xpd = nov.sort((a,b) => (a.title > b.title) ? 1 : ((b.title > a.title) ? -1 : 0))
    return xpd
  }
  const arr2 = orderByName(arr).map((current) => current.title)
  return arr2.splice(0,20)
...
function turnHoursToMinutes(arr) {
  ...
  for(let i = 0; i < arr.length; i++){
    XPD.push(arr[i])
    if(arr[i].duration.slice(-3)===`min`)
    {
    XPD[i].duration = (arr[i].duration.slice(-5).replace("min","").replace(" ","")*1 + (arr[i].duration.slice(0,1)*60))
    }
    else{
      XPD[i].duration = (arr[i].duration.slice(0,1)*60)
    }
  }
  return XPD
}
